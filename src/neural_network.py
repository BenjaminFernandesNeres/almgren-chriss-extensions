...
class StaticScheduleNet(nn.Module):
    """
    Neural network that outputs a liquidation schedule on a fixed time grid.
...
def build_schedule(
    ...
    """
    Build traded quantities, inventory path, and rates from schedule weights.
    ...
    x : torch.Tensor, shape (N,)
        Quantity traded on each time interval.
    q_path : torch.Tensor, shape (N+1,)
        Inventory path at grid points.
    q_mid : torch.Tensor, shape (N,)
        Midpoint inventory on each interval.
    dt : float
        Time step.
    """
    ...
    # Quantity sold on each interval
    x = Q * weights  # sum(x) = Q exactly

    # Inventory after each trade
...
def loss_is(
    ...
    """
    Deterministic IS objective:
        J = eta * ∫ v^2 dt + lam * sigma^2 * ∫ q^2 dt
    """
...
def loss_tc(
    ...
    """
    Deterministic TC objective:
        J = eta * ∫ v^2 dt + lam * sigma^2 * ∫ (Q - q)^2 dt
    """
    t_norm = torch.linspace(0.0, 1.0, N + 1, dtype=TORCH_DTYPE, device=device)[:-1]
    weights = model(t_norm)
...
def train_static_policy(
    ...
    """
    Train a neural static liquidation schedule for IS or TC.
    """
...
def extract_static_strategy(
    ...
    """
    Extract the learned strategy as time grid, inventory path, and rates.
    """
    model.eval()
...
def simulate_pov_hard_cap(
    ...
    for k in range(N):
        ...
        # Positive stochastic market volume
        ...
        # Cannot sell more than remaining inventory
